...
import {
    ...
    NewsPost,
    Washroom,
    WashroomLocationReqPayload,
    isValidDatabaseWashroom,
    isValidNewsPost,
} from "./databasetypes";
...
app.post("/registerUser", express.json(), async (req, res) => {
    try {
        ...
        res
            .status(201)
            .json({ response: "User registered successfully.", token: token });
    } catch (error: any) {
...
app.delete("/deleteUser", express.json(), async (req, res) => {
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
            if (err || jwtDecode<UsernameToken>(token).username != username) {
...
app.delete("/deleteWashroom/:id", express.json(), async (req, res) => {
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
            ...
            const result = await washroomCollection.deleteOne({
                owner_username: jwtDecoded.username,
                id: id,
            });
            ...
            if (result.deletedCount === 1) {
                res
                    .status(200)
                    .json({ response: "Washroom with id " + id + " deleted." });
            } else {
                res.status(404).json({
                    error: "Could not find washroom associated with token to delete.",
                });
            }
        });
    } catch (error: any) {
    ...
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
            ...
            const washrooms = await washroomCollection
                .find({ owner_username: jwtDecoded.username })
                .toArray();
...
app.post("/approveWashroom/:id", express.json(), async (req, res) => {
    try {
        if (!req.headers.authorization) {
            res.status(401).json({ error: "Unauthorized" });
            return;
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
            ...
            const jwtDecoded = jwtDecode<UsernameToken>(token);
            if (jwtDecoded.username != "admin") {
                return res.status(401).send({ error: "Unauthorized" });
            }
            const washroomId = req.params.id;
            const washroom = await washroomCollection.findOneAndUpdate(
                { id: washroomId },
                { $set: { approved: true } },
            );
...
app.get("/getWashroomByLocation/:payload", express.json(), async (req, res) => {
    ...
    try {
        ...
        const washroomCollection = db.collection(COLLECTIONS.Washrooms);
        const washrooms = (await washroomCollection.find().toArray()).filter(
            (washroom) => {
                // if (!washroom.approved) return false;
                let dist = haversineDistance(
                    { lat: washroom.latitude, lon: washroom.longitude },
